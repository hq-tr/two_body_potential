...
# Below is the full two-body PP code on the sphere ============================================================
...
function two_body(N_o::Int64, basis::Vector{BitVector},
				v_list::Vector{Int32}, c_list::Vector{Float64};geom="sphere")
	if geom == "sphere"
		return two_body_sphere(N_o, basis, v_list, c_list)
	elseif geom == "disk"
		return two_body_disk(N_o,basis,v_list,c_list)
	else
		return 
	end
end
...
function two_body_sphere(N_o::Int64, basis::Vector{BitVector},
...
# Below is the full two-body PP code on the disk ============================================================

function update_element_disk!(H_matrix::SparseMatrixCSC{Float64}, N_o::Int64, 
			i::Int64, j::Int64, basis1::BitVector, basis2::BitVector, 
			v_list::Vector{Int32},c_list::Vector{Float64})
	if i == j
		basis = findall(basis1)
...
# This is the matrix for two-body interaction given a basis
function two_body_disk(N_o::Int64, basis::Vector{BitVector},
				v_list::Vector{Int32}, c_list::Vector{Float64})
	dim = length(basis)
	println("The dimension is $(dim)")
	...
	for i in 1:dim
		#print("\rRow $(i+1)\t\t")
		for j in i:dim
			update_element_disk!(H_matrix, N_o, i, j, basis[i], basis[j],v_list,c_list)
			#print("\r$i\t$j\t\t")
		end
	end
#	display(H_matrix)
...
export v1, two_body, two_body_energy,two_body_sphere,two_body_disk, L⁺L⁻, LplusLminus, LL
end
